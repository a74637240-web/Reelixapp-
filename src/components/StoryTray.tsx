...
export default function StoryTray({ onCreatorClick }: StoryTrayProps) {
  ...
  // Video Codec Error Fallbacks for Stories
  const [storyVideoError, setStoryVideoError] = useState<boolean>(false);
  const [storyErrorCount, setStoryErrorCount] = useState<number>(0);

  // Reset error states when moving to another story
  useEffect(() => {
    setStoryVideoError(false);
    setStoryErrorCount(0);
  }, [activeUserStories, activeStoryItemIndex]);
          ...
          <div className="fixed inset-0 z-[9999] bg-[#000000] flex flex-col justify-between h-full w-full select-none animate-fade-in font-sans" style={{ backgroundColor: '#000000' }}>
            ...
            <div className="flex-1 w-full bg-neutral-950 flex items-center justify-center relative overflow-hidden">
              ...
              {activeStory.mediaType === 'image' ? (
                ...
                /* Ambient Visualizer Fallback View for Stories */
                <div className="absolute inset-0 bg-neutral-950 flex flex-col items-center justify-center overflow-hidden z-0">
                  <div 
                    className="absolute inset-0 bg-cover bg-center filter blur-3xl opacity-35 scale-150 transition-all duration-1000"
                    style={{ backgroundImage: `url(${activeUserStories.user.avatar})` }}
                  />
                  ...
                  <div className="relative flex items-center justify-center w-32 h-32">
                    ...
                    <motion.div 
                      ...
                      className="absolute w-5/6 h-5/6 rounded-full border border-purple-500/30"
                    />
                    <div className="relative w-20 h-20 rounded-full overflow-hidden border border-indigo-500/50 shadow-xl flex items-center justify-center bg-neutral-900">
                      <img 
                        src={activeUserStories.user.avatar} 
                        alt="" 
                        className={`w-full h-full object-cover ${!isPaused ? 'animate-[spin_20s_linear_infinite]' : ''}`}
                        referrerPolicy="no-referrer"
                      />
                    </div>
                  </div>
                  
                  <div className="mt-5 text-center px-6 z-10 space-y-1 max-w-[240px]">
                    <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest bg-indigo-500/10 px-2.5 py-0.5 rounded-full border border-indigo-500/20 inline-block">
                      Story Codec Fallback
                    </span>
                    <p className="text-[10px] text-neutral-300">
                      Displaying story in ambient mode due to browser limits
                    </p>
                  </div>
                </div>
              ) : (
                <video
                  ...
                  onError={(e) => {
                    ...
                    if (storyErrorCount < fallbackUrls.length) {
                      const nextFallback = fallbackUrls[storyErrorCount];
                      setStoryErrorCount(prev => prev + 1);
                      e.currentTarget.src = nextFallback;
                    } else {
                      setStoryVideoError(true);
                    }
                  }}
                />
              )}
